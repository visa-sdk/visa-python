# VISA Python bindings
# API spec at https://developer.visa.com/apibrowser
# Authors: John Pham <hprobotic@gmail.com>
...
class APIRequestor(object):
    ...
    def handleApiError(self, rcode, resp):
        try:
            ...
            raise APIError(
                "Invalid response object from API: %r (HTTP response code was %d)" % (rcode, resp))
        ...
        if rcode in [400, 404]:
            ...
            raise CardError(error.get('message'), error.get(
                'param'), error.get('code'))
        else:
    ...
    def request(self, meth, url, params={}):
        ...
        if my_api_key is None:
            raise AuthenticationError(
                'No API key provided.  (HINT: set your API key using "visa.api_key = <API-KEY>".  You can generate API keys from the VISA web interface.  See https://visa.com/api for details, or email support@visa.com if you have any questions.')
        ...
        if _httplib == 'pycurl':
            ...
            raise VISAError(
                "VISA bug discovered: invalid httplib %s.  Please report to support@visa.com" % (_httplib, ))

        try:
            resp = json.loads(rbody)
        except Exception:
            raise APIError(
                "Invalid response body from API: %s (HTTP response code was %d)" % (rbody, rcode))
