...
import visa
from visa import error, util, six
...
try:
    from visa.six.moves import urllib
except ImportError:
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the VISA library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. VISA will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@visa.com. (HINT: running "
...
from visa.six.moves.urllib.parse import urlparse
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the VISA library is falling back to urllib2/urllib "
...
class HTTPClient(object):
    ...
    def _max_network_retries(self):
        from visa import max_network_retries
    ...
    def _record_request_metrics(self, response, request_start):
        ...
        if "Request-Id" in rheaders and visa.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def request(self, method, url, headers, post_data=None):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = visa.ca_bundle_path
        else:
            kwargs["verify"] = False
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with VISA\'s '
    ...
    def _handle_request_error(self, e):
        ...
        if isinstance(e, requests.exceptions.SSLError):
            msg = (
                "Could not verify VISA's SSL certificate.  Please make "
        ...
        # Retry only timeout and connect errors; similar to urllib3 Retry
            ...
            msg = (
                "Unexpected error communicating with VISA.  "
                "If this problem persists, let us know at "
                "support@visa.com."
            )
            err = "%s: %s" % (type(e).__name__, str(e))
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The VISA library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the VISA Python bindings. Please let us know "
                "at support@visa.com." % (url,)
            )
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from VISA."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "VISA.  This is likely due to a bug in VISA. "
                "Please let us know at support@visa.com."
            )
        else:
            msg = (
                "Unexpected error communicating with VISA. If this "
                "problem persists, let us know at support@visa.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def request(self, method, url, headers, post_data=None):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.CAINFO, visa.ca_bundle_path)
        else:
    ...
    def _handle_request_error(self, e):
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to VISA.  Please check your "
                "internet connection and try again.  If this problem "
                "persists, you should check VISA's service status at "
                "https://twitter.com/visastatus, or let us know at "
                "support@visa.com."
            )
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = (
            "Unexpected error communicating with VISA. "
            "If this problem persists, let us know at support@visa.com."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
